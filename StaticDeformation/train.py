...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # objects : 16 * H * W
        # Object Loss
        gt_obj = viewpoint_cam.objects.cuda().long()  # gt_obj : H * W
        logits = classifier(objects)  # logits :  256 * H * W
        # loss_obj.size: torch.Size([])
        # loss_obj: tensor(5.6313, device='cuda:0', grad_fn=<MeanBackward0>)
        loss_obj = cls_criterion(logits.unsqueeze(0), gt_obj.unsqueeze(0)).squeeze().mean()
        # loss_obj_after.size: torch.Size([])
        # loss_obj_after: tensor(1.0155, device='cuda:0', grad_fn=<DivBackward0>)
        ...
        if iteration % opt.reg3d_interval == 0:
            ...
            logits3d = classifier(gaussians._objects_dc.permute(2, 0, 1))  # gaussians._objects_dc N*1*16===>16*N*1
        ...
        with torch.no_grad():
            ...
            training_report(iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene,
                            render, (pipe, background), loss_obj_3d, use_wandb)
